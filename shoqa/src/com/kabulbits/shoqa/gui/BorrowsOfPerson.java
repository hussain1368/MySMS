...
public class BorrowsOfPerson extends JPanel {
	...
	public BorrowsOfPerson(int id, int type)
	{
		pid = id;
		ptype = type;
		...
		String title = Dic.w("person_borrowed_books");
		...
		String cols [] = {"code", "book_title", "reg_no", "cover_no", "delivery_date", "duration", "deadline", "return_date", "returned"};
		for(String col : cols){
		...
		if(Data.PERM_LIBRARY == 2){
			table.getColumnModel().getColumn(4).setCellEditor(new DateEditor() {
				private static final long serialVersionUID = 1L;
				@Override
			...
			table.getColumnModel().getColumn(7).setCellEditor(new DateEditor() {
				...
				public boolean save(Date date, int row) {
					...
					{
						int id = (int) model.getValueAt(row, 0);
						int cover = ((Option) model.getValueAt(row, 3)).key;
			...
			table.getColumnModel().getColumn(8).setCellEditor(new DefaultCellEditor(box)
			{
				private static final long serialVersionUID = 1L;
				@Override
	...
	public void render() 
	{
		...
		Vector<Object[]> rows = data.personBorrows(pid, ptype, unReturned.isSelected(), from, to);
		model.setRowCount(0);
		for(Object [] row : rows){
			model.addRow(row);
		}
	...
	private void delete()
	{
		...
		int id = (int) model.getValueAt(row, 0);
		int cover = 0;
		if(!(boolean) model.getValueAt(row, 8)){
			cover = ((Option) model.getValueAt(row, 3)).key;
		}
